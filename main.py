...
TOKEN = "5688731290:AAFRrSxazuXDZPqNsd-TzGoLPTb6pnbGpSk"

bot = Bot(token=TOKEN, parse_mode=types.ParseMode.HTML)
storage = MemoryStorage()
dp = Dispatcher(bot, storage=storage)

inline_keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton(text="Visit My Site", url="https://t.me/crimsin_kc"))
option_btn = ReplyKeyboardMarkup(resize_keyboard=True).row(KeyboardButton("📽Download Video"), KeyboardButton("🎶Download Music")).add(KeyboardButton("👁Share Phone", request_contact=True), KeyboardButton("Share Location", request_location=True))
...
async def getInfo(msg: types.Message):
    await bot.send_message(msg.from_user.id, 'My name is Asdullo', reply_markup=inline_keyboard)
